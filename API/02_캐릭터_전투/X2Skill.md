...
**확인된 제약:** 과거 반환 데이터 오류가 수정된 API입니다. `itemType`에 따른 반환 차이를 확인하세요.

**형태 예시 — 인자 값은 실제 게임 상태와 관련 상수에 맞게 바꿔야 합니다.**

```lua
local result = X2Skill:GetSkillTooltip(skillIdValue, itemTypeValue)
```
...
**기능:** 재사용 시간 정보를 조회합니다. 게임 또는 UI의 현재 값을 읽는 조회 계열 함수입니다. 원칙적으로 조회 대상 자체를 변경하지 않는 용도로 해석됩니다.

**매개변수**

- `skillId` — `number|string` 추정: skillId에 해당하는 식별자 또는 열거값입니다. 관련 상수 표와 호출 문맥을 함께 확인해야 합니다.
- `ignoreGlobalCooldown` — `any` 추정: ignoreGlobalCooldown에 전달할 값입니다. 자료형과 허용 범위는 원본 덤프에 기록되어 있지 않습니다.
...
```lua
local result = X2Skill:GetCooldown(skillIdValue, ignoreGlobalCooldownValue)
```
...
**기능:** 소환수 재사용 시간 정보를 조회합니다. 게임 또는 UI의 현재 값을 읽는 조회 계열 함수입니다. 원칙적으로 조회 대상 자체를 변경하지 않는 용도로 해석됩니다.
...
```lua
local result = X2Skill:GetMateCooldown(skillIdValue, ignoreGlobalCooldownValue, mateTypeValue)
```
