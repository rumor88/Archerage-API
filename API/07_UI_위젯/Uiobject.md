...
Uiobject UI 객체의 속성, 상태와 동작을 제어합니다.
...
**기능:** Object 유형 정보를 조회합니다. 게임 또는 UI의 현재 값을 읽는 조회 계열 함수입니다. 원칙적으로 조회 대상 자체를 변경하지 않는 용도로 해석됩니다.
...
```lua
local result = widget:GetObjectType()
```
...
**기능:** Valid UI Object 여부를 확인합니다. 조건 충족 여부를 확인하는 판정 계열 함수입니다. 이름상 참/거짓 값을 반환하는 것으로 추정됩니다.
...
```lua
local result = widget:IsValidUIObject()
```
...
**기능:** 이름 정보를 조회합니다. 게임 또는 UI의 현재 값을 읽는 조회 계열 함수입니다. 원칙적으로 조회 대상 자체를 변경하지 않는 용도로 해석됩니다.
