...
Textbox UI 객체의 속성, 상태와 동작을 제어합니다.
...
**기능:** Longest Line Width 정보를 조회합니다. 게임 또는 UI의 현재 값을 읽는 조회 계열 함수입니다. 원칙적으로 조회 대상 자체를 변경하지 않는 용도로 해석됩니다.
...
```lua
local result = widget:GetLongestLineWidth()
```
...
**기능:** Line Length 정보를 조회합니다. 게임 또는 UI의 현재 값을 읽는 조회 계열 함수입니다. 원칙적으로 조회 대상 자체를 변경하지 않는 용도로 해석됩니다.

**매개변수**

- `lineNum` — `any` 추정: lineNum에 전달할 값입니다. 자료형과 허용 범위는 원본 덤프에 기록되어 있지 않습니다.
...
```lua
local result = widget:GetLineLength(lineNumValue)
```
...
**기능:** 텍스트 Auto Width 값을 설정하거나 변경합니다. 전달한 값으로 설정 또는 상태를 변경하는 함수입니다. 호출 직후 UI나 클라이언트 상태에 반영될 수 있습니다.

**매개변수**

- `initWidth` — `number` 추정: initWidth에 해당하는 숫자 값입니다. 유효 범위와 시작 번호는 원본 덤프에 기록되어 있지 않습니다.
- `text` — `string` 추정: 표시하거나 처리할 문자열입니다.
...
```lua
widget:SetTextAutoWidth(1, "text", 1)
```
...
**기능:** Line Height 값을 설정하거나 변경합니다. 전달한 값으로 설정 또는 상태를 변경하는 함수입니다. 호출 직후 UI나 클라이언트 상태에 반영될 수 있습니다.
...
**기능:** Strike Through 값을 설정하거나 변경합니다. 전달한 값으로 설정 또는 상태를 변경하는 함수입니다. 호출 직후 UI나 클라이언트 상태에 반영될 수 있습니다.
...
**기능:** Under Line 값을 설정하거나 변경합니다. 전달한 값으로 설정 또는 상태를 변경하는 함수입니다. 호출 직후 UI나 클라이언트 상태에 반영될 수 있습니다.
